...
merge_DHMR_diff <- function(compare_list,tile_width, min_diff=0.01){
  ...
  worker_fun <- function(x){
    ...
    fileName <- paste0("Stage_",pre_stage,"_to_",pro_stage,".",tile_width,".DMR.metilene.out")
    ...
    if(file.info(fileName)$size==0) return(NULL)
      ...
      dt_summary <- dt %>% filter(meth_g1 >=0.001 & meth_g2 >=0.001) %>%
        ...
        mutate(category=case_when(
          qval < 0.05 & meth_diff >= min_diff ~ "decreasing",
          qval >=0.05 | (qval < 0.05 & abs(meth_diff) < min_diff ) ~"stable",
          qval < 0.05 & meth_diff <= -min_diff ~"increasing"
        ), cmp=paste0(pre_stage, "->", pro_stage)
        )
